...
When a feature requires work on existing content, add a stable, cumulative check to `scripts/indexx_upgrade.py` with its user-facing purpose, affected scope/counts, observable completion condition, and next action. Keep applicable older checks so users skipping releases or deferring work still see them. Installation revision and dismissed notices are never evidence of content completion. Distinguish deterministic derived-data refreshes from bot review or paid work. Test old libraries, already-current installations with pending work, reviewed empty results, malformed sources, and check-only preservation. The current person check validates explicit reviewed metadata, not semantic correctness of the whole wiki. Include new runtime dependencies in the installer's support list, and keep the update skill, follow-up skill, profile routing, README, and public bundle aligned.
