...
Keep the trusted repository/channel/workflow in the updater and its skill consistent when intentionally changing them. The separate `NYLLON-SOFTWARE/indexx-codex` bookmark-export plugin is not an update source for this Grok Bot. The installer distributes the helper so subsequent requests can use an already verified local copy; bots predating it need the one-time bootstrap described in the README. Update the skill, profile routing, and exported bundle together, and preserve read-only behavior for check-only requests. Never claim a GitHub merge automatically updates saved bot definitions or recipient copies of a Grok template.
