...
The repository now includes `.agents/plugins/marketplace.json`, exposing `indexx` in the **indexx** marketplace. Its local source is `./`, so it loads the existing root plugin and shared skill without maintaining a second copy. This matches [Compound Engineering's marketplace layout](https://github.com/EveryInc/compound-engineering-plugin/blob/main/.agents/plugins/marketplace.json) and OpenAI's rule that source paths resolve relative to the repository root. OpenAI explicitly supports GitHub-backed custom marketplaces for repository and CLI distribution. [Marketplace documentation](https://developers.openai.com/plugins/build/plugins#build-your-own-curated-plugin-list)

Use **Plugins → Create menu → Add marketplace** with source `NYLLON-SOFTWARE/indexx`, Git ref `main`, and no sparse paths, then install the plugin. The [README](../README.md#install-in-the-codex-app) has the complete app and optional CLI steps. No hosted server or public directory review is required for a custom marketplace. The repository is public at `NYLLON-SOFTWARE/indexx`, so anyone can add the marketplace; collection still requires the user's own authorized Instagram session.
