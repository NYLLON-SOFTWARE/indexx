...
## Repository marketplace update — 2026-09-11

The repository now includes `.agents/plugins/marketplace.json`, exposing `indexx-instagram-skill` in the **INDEXX Instagram** marketplace. Its local source is `./`, so it loads the existing root plugin and shared skill without maintaining a second copy. This matches [Compound Engineering's marketplace layout](https://github.com/EveryInc/compound-engineering-plugin/blob/main/.agents/plugins/marketplace.json) and OpenAI's rule that source paths resolve relative to the repository root. OpenAI explicitly supports GitHub-backed custom marketplaces for repository and CLI distribution. [Marketplace documentation](https://developers.openai.com/plugins/build/plugins#build-your-own-curated-plugin-list)

Use **Plugins → Create menu → Add marketplace** with source `NYLLON-SOFTWARE/indexx-instagram-skill`, Git ref `main`, and no sparse paths, then install the plugin. The [README](../README.md#install-in-the-codex-app) has the complete app and optional CLI steps. No hosted server or public directory review is required for a custom marketplace. The repository remains private, so users need GitHub read access; the catalog does not change repository permissions.

Native Codex catalog discovery was checked with temporary command-line configuration: it listed the correct plugin as available and uninstalled. This check did not register the marketplace permanently, install the plugin, or exercise Instagram. Public directory publication and a complete browser export remain separate work.
