...
Use the watch helper's intrinsic video sizing: portrait, landscape, and square players keep the actual video's aspect ratio, bounded by the card width and player height limit. Do not force portrait clips into full-width players or crop/stretch the video. The browser preloads local metadata to learn the dimensions before playback; autoplay remains off. After a watch-renderer update, regenerate a requested saved watch page with the same filters and name so it receives the new layout; refreshing an old HTML file alone does not update its template.
