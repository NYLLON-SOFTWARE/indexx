...
Run these commands on the registered Mac, with the confirmed library path:

```bash
python3 scripts/indexx_search.py status --root "$LIBRARY_ROOT"
python3 scripts/indexx_search.py build --root "$LIBRARY_ROOT"
```
...
```bash
python3 scripts/indexx_search.py query --root "$LIBRARY_ROOT" --person "Alan Watts" --role speaker --type video --all
```
...
```bash
python3 scripts/indexx_search.py query --root "$LIBRARY_ROOT" --text "anxiety" --all
python3 scripts/indexx_search.py query --root "$LIBRARY_ROOT" --text "anxiety" --person "Alan Watts" --role speaker --type video --all
```
...
```bash
python3 scripts/indexx_watch.py --root "$LIBRARY_ROOT" --person "Alan Watts" --role speaker --type video --name alan-watts
```
