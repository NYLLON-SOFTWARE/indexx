...
compatibility: Runs through a browser-capable agent or sidebar extension with authenticated tab access, JavaScript data processing, and checkpoint/export tools. No Python, Node.js installation, shell, or external packages are required to use the skill.
---
...
Run this workflow from the agent's browser extension/sidebar or equivalent browser tools. Verify that it can inspect rendered link URLs, navigate and scroll the authenticated tab, process batches in JavaScript, and retain/export results. Read [references/runtime.md](references/runtime.md) when selecting the host's tools or when a capability is missing. Use the extension's existing capabilities; do not ask the user to install a programming language or run terminal commands.

Resolve bundled paths relative to this `SKILL.md` directory. Keep checkpoints in the host's supported private artifact/storage facility, outside the skill installation. The skill supplies instructions and a pure JavaScript data helper; the extension supplies browser access and delivery tools.
...
Read [references/browser-collection.md](references/browser-collection.md) for the tested DOM extraction shape and checkpoint format before collecting.

1. Confirm the correct **All Posts** grid and identify its content container from a fresh snapshot. Record the source URL and capture time. For a fresh run, begin at the top; for a resume, load existing checkpoints and re-traverse from the top, deduplicating until reaching new items.
...
3. Scroll the actual grid container by roughly one viewport with overlap, using supported browser actions. Re-read page state and capture links after each scroll/load. Where the host permits DOM evaluation, use it for read-only extraction and scroll metrics; use browser actions for scrolling.
...
5. At the apparent bottom, confirm no pending loading and unchanged tail IDs/scroll extent across at least three settled scroll-and-observe cycles. Label this `end-observed`, meaning the accessible UI appeared exhausted, not a server-verified account total. A persistent spinner, error, challenge, repeated stall away from the bottom, or user limit means `partial`; checkpoint and explain the exact stopping reason. Allow one ordinary retry of a transient loading failure, then stop if the same blocker persists.
...
Deliver `saved-items.json`, `saved-items.csv`, and `saved-ids.txt` from `files` using the extension's supported download/artifact tools. Do not claim files were saved if only their contents were generated. If file attachments are unavailable, offer the exact contents in labeled copyable blocks when they fit; otherwise preserve the checkpoint and report the delivery limitation without truncating the export.
